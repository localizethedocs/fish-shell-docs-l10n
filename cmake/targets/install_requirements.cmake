...
if     (VERSION_OF_GCC STREQUAL "[none]")
    set(PACKAGE_OF_GCC "")
    set(PACKAGE_OF_GXX "")
elseif (VERSION_OF_GCC STREQUAL "")
    set(PACKAGE_OF_GCC "conda-forge::gcc")
    set(PACKAGE_OF_GXX "conda-forge::gxx")
else()
    set(PACKAGE_OF_GCC "conda-forge::gcc=${VERSION_OF_GCC}")
    set(PACKAGE_OF_GXX "conda-forge::gxx=${VERSION_OF_GCC}")
endif()
if     (VERSION_OF_BINUTILS STREQUAL "[none]")
    set(PACKAGE_OF_BINUTILS "")
elseif (VERSION_OF_BINUTILS STREQUAL "")
    set(PACKAGE_OF_BINUTILS "conda-forge::binutils")
else()
    set(PACKAGE_OF_BINUTILS "conda-forge::binutils=${VERSION_OF_BINUTILS}")
endif()
if     (VERSION_OF_CMAKE STREQUAL "[none]")
    set(PACKAGE_OF_CMAKE "")
elseif (VERSION_OF_CMAKE STREQUAL "")
    set(PACKAGE_OF_CMAKE "conda-forge::cmake")
else()
    set(PACKAGE_OF_CMAKE "conda-forge::cmake=${VERSION_OF_CMAKE}")
endif()
...
if     (VERSION_OF_SPHINX STREQUAL "[none]")
    set(PACKAGE_OF_SPHINX "")
elseif (VERSION_OF_SPHINX STREQUAL "")
    set(PACKAGE_OF_SPHINX "conda-forge::sphinx")
else()
    set(PACKAGE_OF_SPHINX "conda-forge::sphinx=${VERSION_OF_SPHINX}")
endif()
if     (VERSION_OF_RUST STREQUAL "[none]")
    set(PACKAGE_OF_RUST "")
elseif (VERSION_OF_RUST STREQUAL "")
    set(PACKAGE_OF_RUST "conda-forge::rust")
else()
    set(PACKAGE_OF_RUST "conda-forge::rust=${VERSION_OF_RUST}")
endif()
remove_cmake_message_indent()
message("")
execute_process(
...
message(STATUS "Removing directory '${PROJ_OUT_REPO_DIR}/build/'...")
if (EXISTS "${PROJ_OUT_REPO_DIR}/build")
    file(REMOVE_RECURSE "${PROJ_OUT_REPO_DIR}/build")
    remove_cmake_message_indent()
    message("")
    message("Directory '${PROJ_OUT_REPO_DIR}/build/' exists.")
    message("Removed '${PROJ_OUT_REPO_DIR}/build/'.")
    message("")
    restore_cmake_message_indent()
else()
    remove_cmake_message_indent()
    message("")
    message("Directory '${PROJ_OUT_REPO_DIR}/build/' does NOT exist.")
    message("No need to remove '${PROJ_OUT_REPO_DIR}/build/'.")
    message("")
    restore_cmake_message_indent()
endif()
...
message(STATUS "Configuring the fish-shell project...")
if (CMAKE_HOST_LINUX)
    set(ENV_PATH                    "${PROJ_CONDA_DIR}/bin:$ENV{PATH}")
    set(ENV_LD_LIBRARY_PATH         "${PROJ_CONDA_DIR}/lib:$ENV{LD_LIBRARY_PATH}")
    set(ENV_CMAKE_PREFIX_PATH       "${PROJ_CONDA_DIR}")
    set(ENV_CMAKE_PROGRAM_PATH      "${PROJ_CONDA_DIR}/bin")       # find_program
    set(ENV_CMAKE_INCLUDE_PATH      "${PROJ_CONDA_DIR}/include")   # find_file
    set(ENV_CMAKE_LIBRARY_PATH      "${PROJ_CONDA_DIR}/lib")       # find_library
    set(ENV_CMAKE_INSTALL_PREFIX    "${PROJ_CONDA_DIR}")
    set(ENV_VARS_OF_SYSTEM          PATH=${ENV_PATH}
                                    LD_LIBRARY_PATH=${ENV_LD_LIBRARY_PATH}
                                    CMAKE_PREFIX_PATH=${ENV_CMAKE_PREFIX_PATH}
                                    CMAKE_PROGRAM_PATH=${ENV_CMAKE_PROGRAM_PATH}
                                    CMAKE_INCLUDE_PATH=${ENV_CMAKE_INCLUDE_PATH}
                                    CMAKE_LIBRARY_PATH=${ENV_CMAKE_LIBRARY_PATH}
                                    CMAKE_INSTALL_PREFIX=${ENV_CMAKE_INSTALL_PREFIX})
else()
    message(FATAL_ERROR "Invalid OS platform. (${CMAKE_HOST_SYSTEM_NAME})")
endif()
if     (VERSION MATCHES "^(current)$")
    set(ENABLE_DOCS_OPTION  WITH_DOCS)
elseif (VERSION MATCHES "^([0-9]+)$"      AND
        VERSION VERSION_LESS_EQUAL    "4" AND
        VERSION VERSION_GREATER_EQUAL "4")
    set(ENABLE_DOCS_OPTION  WITH_DOCS)
elseif (VERSION MATCHES "^([0-9]+)$"      AND
        VERSION VERSION_LESS_EQUAL    "3" AND
        VERSION VERSION_GREATER_EQUAL "3")
    set(ENABLE_DOCS_OPTION  BUILD_DOCS)
else()
...
message("")
execute_process(
    COMMAND ${CMake_EXECUTABLE} -E env
            ${ENV_VARS_OF_SYSTEM}
            ${CMake_EXECUTABLE}
            -S ${PROJ_OUT_REPO_DIR}
            -B ${PROJ_OUT_REPO_DIR}/build
            -D ${ENABLE_DOCS_OPTION}=OFF
...
message(STATUS "Building the fish-shell project to generate fish/fish_indent executables...")
if (CMAKE_HOST_LINUX)
    set(ENV_PATH                "${PROJ_CONDA_DIR}/bin:$ENV{PATH}")
    set(ENV_LD_LIBRARY_PATH     "${PROJ_CONDA_DIR}/lib:$ENV{LD_LIBRARY_PATH}")
    set(ENV_VARS_OF_SYSTEM      PATH=${ENV_PATH}
